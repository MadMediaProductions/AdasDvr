...
class AsyncServiceHelper {
    ...
    public static boolean initOpenCV(String Version, Context AppContext, LoaderCallbackInterface Callback) {
        ...
        if (AppContext.bindService(new Intent("org.opencv.engine.BIND"), helper.mServiceConnection, 1)) {
            return true;
        }
    ...
    protected static void InstallService(Context AppContext, LoaderCallbackInterface Callback) {
        ...
        Callback.onPackageInstall(1, new InstallCallbackInterface(Callback, AppContext) {
            private LoaderCallbackInterface mUserAppCallback;
            private final /* synthetic */ Context val$AppContext;
    ...
    public boolean initOpenCVLibs(String Path, String Libs) {
        ...
        if (Libs == null || Libs.length() == 0) {
            return true & loadLibrary(String.valueOf(Path) + File.separator + "libopencv_java.so");
        }
        Log.d(TAG, "Trying to load libs by dependency list");
        StringTokenizer splitter = new StringTokenizer(Libs, ";");
        while (splitter.hasMoreTokens()) {
            result &= loadLibrary(String.valueOf(Path) + File.separator + splitter.nextToken());
        }
        return result;
    }
}
