...
public class HtmlToPlainText {
    ...
    private class FormattingVisitor implements NodeVisitor {
        ...
        public void head(Node node, int depth) {
            ...
            if (node instanceof TextNode) {
                ...
                if (StringUtil.in(name, "p", "h1", "h2", "h3", "h4", "h5", "tr")) {
                    append(DoCmdUtil.COMMAND_LINE_END);
                }
            }
        }
        ...
        private void append(String text) {
            boolean last;
            if (text.startsWith(DoCmdUtil.COMMAND_LINE_END)) {
                this.width = 0;
            }
            if (text.equals(" ")) {
                if (this.accum.length() != 0) {
                    if (StringUtil.in(this.accum.substring(this.accum.length() - 1), " ", DoCmdUtil.COMMAND_LINE_END)) {
                        return;
                    }
                } else {
                    return;
            ...
            if (text.length() + this.width > 80) {
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    if (word.length() + this.width > 80) {
                        this.accum.append(DoCmdUtil.COMMAND_LINE_END).append(word);
