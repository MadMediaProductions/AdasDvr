...
public final class QueueDrainHelper {
    ...
    public static <T> boolean postCompleteRequest(long n, Subscriber<? super T> actual, Queue<T> queue, AtomicLong state, BooleanSupplier isCancelled) {
        ...
        do {
            r = state.get();
        } while (!state.compareAndSet(r, (COMPLETED_MASK & r) | BackpressureHelper.addCap(r & REQUESTED_MASK, n)));
        if (r != COMPLETED_MASK) {
            return false;
        }
        postCompleteDrain(COMPLETED_MASK | n, actual, queue, state, isCancelled);
        return true;
    }
