...
public final class USBMonitor {
    ...
    public synchronized void register() throws IllegalStateException {
        if (this.destroyed) {
            ...
            this.mAsyncHandler.postDelayed(this.mDeviceCheckRunnable, 1000);
        }
    }
            ...
            if (filters == null || filters.isEmpty()) {
                ...
                for (UsbDevice device2 : deviceList.values()) {
                    ...
                    while (true) {
                        ...
                        if (filter != null && filter.matches(device2)) {
                            if (!filter.isExclude && filter.mProductId == 25450 && filter.mVendorId == 3141) {
                                result.add(device2);
                            }
                        }
                    }
    ...
    private void myRequestPermission(UsbDevice device) {
        try {
            Log.e("myRequestPermission", "start request");
            IUsbManager usbManager = IUsbManager.Stub.asInterface(ServiceManager.getService("usb"));
            if (usbManager != null) {
                Log.e("myRequestPermission", "on request");
                usbManager.grantDevicePermission(device, ((Context) this.mWeakContext.get()).getPackageManager().getApplicationInfo(((Context) this.mWeakContext.get()).getPackageName(), 0).uid);
                processConnect(device);
                Log.e("myRequestPermission", "end request");
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (this.mConnection != null) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (intfs != null) {
                        int m = intfs.size();
                        for (int j = 0; j < m; j++) {
                            this.mConnection.releaseInterface(intfs.valueAt(j));
                        }
                        intfs.clear();
                    }
                }
